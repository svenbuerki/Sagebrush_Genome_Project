...
timetab <- data.frame("Objective" = c(rep("Ref. genome", 2), rep("Comparative genomics",2), rep("Common gardens", 3), rep("Landscape genomics",3), rep("Individual lines", 2)), "Task" = c("Seq., phasing & assembly", "Annotation", "Re-sequencing","Pan-genomics","GxE exp.", "RNA-seq", "Transcriptomics", "Id. adaptive loci", "Baiting", "Modeling", "Propagation", "Maintaining lines"), "Start_Date" = lubridate::mdy(rep("NA", 12)), "End_Date" = lubridate::mdy(rep("NA", 12)))
...
###
#Propagation
timetab$Start_Date[11] <- as.Date("02-13-20", tryFormats = c("%m-%d-%y"))
timetab$End_Date[11] <- as.Date("04-07-21", tryFormats = c("%m-%d-%y"))
#Maintaining lines
timetab$Start_Date[12] <- timetab$End_Date[11] + lubridate::weeks(1)
timetab$End_Date[12] <- timetab$End_Date[10]

#Add col with y height
timetab$Y <- c(rep(2.5, 2), 2, 2, 1.5, 1.3, 1, rep(0.5, 3), rep(3,2))
#Add color
# Blue = wet-lab
# orange = bioinformatics
# brown = wet/bioinfo
timetab$col <- c("brown", "orange", "blue", "orange", "blue", "blue", "orange", "orange", "blue", "orange", "blue", "blue")
...
text(x = as.Date(timetab$Start_Date) + floor((as.Date(timetab$End_Date)-as.Date(timetab$Start_Date))/2)  , y = timetab$Y+0.15, paste(as.vector(timetab$Task), "\n", "(", timetab$duration, "m.)", sep=""), srt=10, cex=.7)

#Arrows with fiscal years
#Y2
arrows(x0 = lubridate::floor_date(min(timetab$Start_Date), "month"), x1 = as.Date("09-30-20", tryFormats = c("%m-%d-%y")), y0 = 0.2, y1 = 0.2, length = 0.1, angle = 10)
text(x = lubridate::floor_date(min(timetab$Start_Date), "month") + floor((as.Date("09-30-20", tryFormats = c("%m-%d-%y"))-lubridate::floor_date(min(timetab$Start_Date), "month"))/2), y = 0.3, "Year 2", cex = 0.8)

#Y3
arrows(x0 = as.Date("10-01-20", tryFormats = c("%m-%d-%y")), x1 = as.Date("09-30-21", tryFormats = c("%m-%d-%y")), y0 = 0.2, y1 = 0.2, length = 0.1, angle = 10, code = 3)
text(x = as.Date("10-01-20", tryFormats = c("%m-%d-%y")) + floor((as.Date("09-30-21", tryFormats = c("%m-%d-%y"))-as.Date("10-01-20", tryFormats = c("%m-%d-%y")))/2), y = 0.3, "Year 3", cex = 0.8)
...
arrows(x0 = timetab$End_Date[11]-40, x1 = timetab$Start_Date[1]-2, y0 = timetab$Y[11], y1 = timetab$Y[1], score = 1, length = 0.1, lwd = 1.5, lty = 6, angle = 20, col = "black")

#Propagation to Re-sequencing
arrows(x0 = timetab$End_Date[11]-50, x1 = timetab$Start_Date[3]-2, y0 = timetab$Y[11], y1 = timetab$Y[3], score = 1, length = 0.1, lwd = 1.5, lty = 6, angle = 20, col = "black")

#Propagation to GxE
arrows(x0 = timetab$End_Date[11]-60, x1 = timetab$Start_Date[5]-2, y0 = timetab$Y[11], y1 = timetab$Y[5], score = 1, length = 0.1, lwd = 1.5, lty = 6, angle = 20, col = "black")
