...
The predicted timetable for completion of the sagebrush genome project to support objectives of the GEM3 project is provided in Figure \@ref(fig:timetab). Assembling and annotating the sagebrush genome will serve to assess adaptive capacity of sagebrush populations to cope with drought and heat driven by climate change. This will be done by conducting common garden experiments on [individual lines](Individual_lines.html) of known genomes (serving establishing reference genome and whole genome re-sequencing) and then at landscape level using adaptive loci (sequenced using a baiting approach) on plants from across the climatic and geographic range of the species.  

```{r timetab, warning = F, echo=F, fig.cap= "Timetable associated to production of data for the sagebrush genome project. Please see text for more details.", out.width = '100%'}
#Timetable

timetab <- data.frame("Objective" = c(rep("Ref. genome", 3), rep("Common gardens", 3), rep("Landscape genomics",3)), "Task" = c("Seq., phasing & assembly", "Annotation", "Re-sequencing", "GxE exp.", "RNA-seq", "Transcriptomics", "Identify adaptive loci", "Baiting", "Modeling"), "Start_Date" = lubridate::mdy(rep("NA", 9)), "End_Date" = lubridate::mdy(rep("NA", 9)))
...
#When ready for GxE
timetab$Start_Date[4] <- as.Date("04-01-21", tryFormats = c("%m-%d-%y")) + lubridate::weeks(2)

#When GxE end?
timetab$End_Date[4] <- as.Date("04-09-21", tryFormats = c("%m-%d-%y")) + lubridate::weeks(28)

#RNA-Seq
timetab$Start_Date[5] <- as.Date("04-01-21", tryFormats = c("%m-%d-%y")) + lubridate::weeks(20)
timetab$End_Date[5] <- timetab$End_Date[4] + lubridate::weeks(10)

#Transcriptomics
timetab$Start_Date[6] <- timetab$Start_Date[5] + lubridate::weeks(10)
timetab$End_Date[6] <- timetab$End_Date[5] + lubridate::weeks(20)
...
timetab$Start_Date[7] <- timetab$Start_Date[6]
timetab$End_Date[7] <- timetab$End_Date[5] + lubridate::weeks(8)

#Baiting
timetab$Start_Date[8] <- timetab$End_Date[7]
timetab$End_Date[8] <- timetab$Start_Date[8] + lubridate::weeks(8)

#Modeling
timetab$Start_Date[9] <- timetab$End_Date[8]
timetab$End_Date[9] <- timetab$Start_Date[9] + lubridate::weeks(16)

#Add col with y height
timetab$Y <- c(rep(2.5, 2), 2.2, 1.5, 1.2, 1, rep(0.5, 3))
#Add color
# Blue = wet-lab
# orange = bioinformatics
# brown = wet/bioinfo
timetab$col <- c("brown", "orange", "brown", "blue", "blue", "orange", "orange", "blue", "orange")
...
#Plot timetable
plot(x=c(lubridate::floor_date(min(timetab$Start_Date), "month"), lubridate::round_date(max(timetab$End_Date), "month")), y= c(0,3), type="n", axes = F, ylab = "", xlab = "")

#Abline to separate years
abline(v =  as.Date("12-01-21", tryFormats = c("%m-%d-%y")))
text(x = as.Date(timetab$Start_Date[1]) + abs(floor((as.Date(timetab$Start_Date[1])-as.Date("12-01-21", tryFormats = c("%m-%d-%y")))/2)), y = 0.01, cex = 0.8, "2021")
text(x = as.Date("12-01-21", tryFormats = c("%m-%d-%y")) + abs(floor(as.Date("12-01-21", tryFormats = c("%m-%d-%y")) - as.Date(max(timetab$End_Date)))/2), y = 0.01, cex = 0.8, "2022")

#Add customed axes
#Y axis = Individual lines
axis(side = 2, at = c(0.5, 1.5, 2.5), las = 2, labels = rev(gsub(" ", "\n", unique(timetab$Objective))), cex= .6, pos=lubridate::floor_date(min(timetab$Start_Date), "month"))
...
arrows(x0 = timetab$Start_Date[6]+20, x1 = timetab$Start_Date[6]+20, y0 = timetab$Y[6], y1 = timetab$Y[2], score = 1, length = 0.1, lwd = 1.5, lty = 6, angle = 20, col = "black", code = 3)

#Annotation to adaptive loci
arrows(x0 = timetab$End_Date[2]-52, x1 = timetab$End_Date[2]-35, y0 = timetab$Y[2], y1 = timetab$Y[7], score = 1, length = 0.1, lwd = 1.5, lty = 6, angle = 20, col = "black")

#Transcriptomics to adaptive loci
arrows(x0 = timetab$End_Date[6]-10, x1 = timetab$End_Date[7]-30, y0 = timetab$Y[6], y1 = timetab$Y[7], score = 1, length = 0.1, lwd = 1.5, lty = 6, angle = 20, col = "black")
