...
Estimate 2C genome size of *Artemisia tridentata* based on leaf material using the one-step protocol with general purpose buffer (GPB) by @Loureiro2007 and the protocol described in @Pellicer2014. 

## Challenge

Propidium iodide is widely used in a range of medical applications, so I assume the laser fitted in the instrument would also allow estimating nuclear DNA content in plant tissues. However, most medical applications aim at separating nuclei populations rather than detecting changes in small fluorescence intensity for estimating DNA contents. One thing we need to bear in mind is the coefficient of variation of the fluorescence peaks, which should be below 5%, and the measurements taken using a linear scale rather that a logarithmic scale. This might be the trickiest point depending on the instrument we are using at BSU.
