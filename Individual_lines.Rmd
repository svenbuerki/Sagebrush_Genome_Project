...
OUTChamber <- matrix(ncol=4, nrow=2)
colnames(OUTChamber) <- c("Chamber", "Carrying capacity","Total", "Percent")
OUTChamber[,1] <- unique(Growth$Chamber)

for(i in 1:nrow(OUTChamber)){
  #Subset to Chamber
  tmp <- subset(Growth, Growth$Chamber == OUTChamber[i,1])
  #Infer carrying capacity
  OUTChamber[i,2] <- CarDat$Carrying_capacity[which(CarDat$Type == OUTChamber[i,1])]
  #Infer #boxes
  OUTChamber[i,3] <- sum(tmp$Total)
  #Infer percent
  OUTChamber[i,4] <- round(100*(as.numeric(OUTChamber[i,3])/ as.numeric(OUTChamber[i,2])),2)
}
...
OUTChamber$Percent <- color_bar(color = "gray")(OUTChamber$Percent)
